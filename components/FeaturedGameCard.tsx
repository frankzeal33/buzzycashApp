...
const FeaturedGameCard = ({item, handlePress, index}: {item: any; handlePress: () => void; index: number}) => {
  ...
  return (
    <Pressable onPress={() => router.push(item.route)} className={`w-[48%] rounded-lg border border-brown-500 bg-gray-100 overflow-hidden`}>
        <View className='flex-row gap-1 px-2 py-5 items-center'>
            ...
            <Text className='font-mmedium flex-1'>{item.description}</Text>
        </View>
