    ...
    <Pressable onPress={handlePress} className='border-b-2 w-full h-28' style={{ backgroundColor: theme.colors.darkGray, borderColor: theme.colors.background}}>
        <View className="flex-1 justify-between w-full flex-row items-start gap-3 rounded-lg px-4 py-2 my-1">
          <View className="items-start flex-row gap-2 max-w-[50%]">
            ...
            <View>
              <Text className="font-mbold text-base capitalize" numberOfLines={1} style={{ color: theme.colors.text}}>{item?.name} -</Text>
              <Text className="font-mregular text-sm my-1" numberOfLines={1} style={{ color: theme.colors.text}}>{moment(item?.createdAt).format('llll')}</Text>
              <Text className="font-mmedium text-base mt-1" numberOfLines={1} style={{ color: theme.colors.text}}>#{item?.id}</Text>
            </View>
          </View>

          <View className='items-end justify-end max-w-[35%]'>
            <Text className="font-bold text-base" style={{ color: theme.colors.text}}>{displayCurrency(Number(item?.amount), 'NGN')}</Text>
            <Text className="font-light text-sm"  style={{ color: theme.colors.text}}>Bal: {displayCurrency(Number(item?.amount), 'NGN')}</Text>
          </View>
        </View>
    </Pressable >
  )
